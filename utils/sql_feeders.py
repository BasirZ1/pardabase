...
async def insert_new_product(image, name, category_index, cost_per_metre, price, description):
    try:
        async with connection_context() as conn:
            ...
            """
            product_code = await conn.fetchval(sql_insert, image, name, category_index, cost_per_metre, price,
                                               description)
            return product_code
...
async def add_new_user_ps(full_name, username, password, level, photo):
    """
    ...
    try:
        async with connection_context() as conn:
            ...
            sql_insert = """
                INSERT INTO users (full_name, username, password, level, photo) 
                VALUES ($1, LOWER($2), $3, $4, $5)
            """

            await conn.execute(sql_insert, full_name, username, hashed_password, level, photo)
            return True
